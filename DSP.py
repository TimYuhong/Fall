...
from dsp.range_processing import range_processing # 距离FFT处理
...
# Radar.cfg: profileCfg第10参数=128, frameCfg每帧64 chirps(每TX), channelCfg 15 7 0 → 4RX 3TX
NUM_CHIRPS = 64
NUM_ADC_SAMPLES_RAW = 128   # Radar.cfg profileCfg 第10参数=128（复数采样点）
NUM_ADC_SAMPLES = 128        # range FFT输出bin数与采样点一致
# Radar.cfg: profileCfg slope=65 MHz/us, dig_out_sample_rate=2500 ksps
# range_resolution = c / (2 * slope * Ts * N) = 3e8 / (2 * 65e6 * (1/2500e3) * 128) ≈ 0.0449 m
RANGE_RESOLUTION = 0.0449
...
# 计算分辨率
# Radar.cfg: dig_out_sample_rate=2500 ksps, freq_slope_const=65 MHz/us
range_resolution, bandwidth = dsp.range_resolution(NUM_ADC_SAMPLES_RAW, dig_out_sample_rate=2500, freq_slope_const=65)
...
def apply_runtime_config(runtime_cfg=None):
    ...
    global NUM_TX, NUM_RX, NUM_CHIRPS
    global NUM_ADC_SAMPLES_RAW, NUM_ADC_SAMPLES
    global RANGE_RESOLUTION, DOPPLER_RESOLUTION
    global ANGLE_RANGE, ANGLE_BINS, BINS_PROCESSED
    global numRangeBins, numDopplerBins
    global range_resolution, bandwidth, doppler_resolution
    global range_azimuth, range_elevation, azimuth_elevation
    global num_vec, steering_vec

    NUM_TX = int(runtime_cfg.get("num_tx", NUM_TX))
    NUM_RX = int(runtime_cfg.get("num_rx", NUM_RX))
    NUM_CHIRPS = int(runtime_cfg.get("num_doppler_bins", NUM_CHIRPS))
    NUM_ADC_SAMPLES_RAW = int(runtime_cfg.get("num_adc_samples_raw", NUM_ADC_SAMPLES_RAW))
    NUM_ADC_SAMPLES = int(runtime_cfg.get("num_range_bins", NUM_ADC_SAMPLES))
    RANGE_RESOLUTION = float(runtime_cfg.get("range_resolution_m", RANGE_RESOLUTION))
    DOPPLER_RESOLUTION = float(runtime_cfg.get("doppler_resolution_mps", DOPPLER_RESOLUTION))

    aoa_fov = runtime_cfg.get("aoa_fov") or {}
    azimuth_fov = aoa_fov.get("azimuth", (-ANGLE_RANGE, ANGLE_RANGE))
    ANGLE_RANGE = max(abs(float(azimuth_fov[0])), abs(float(azimuth_fov[1])))
    ...
    bandwidth = dsp.range_resolution(NUM_ADC_SAMPLES_RAW, dig_out_sample_rate=2500, freq_slope_const=65)[1]
...
def RDA_Time(adc_data, window_type_1d=None, clutter_removal_enabled=True, CFAR_enable=False, axis=-1):
    ...
    rti_array = np.reshape(rti_framearray, (1, -1, numRangeBins))#chirps adcnum
    # (num_chirps_per_frame, num_range_bins, num_rx_antennas)
    rti_array_out = np.transpose(rti_array, [1, 2, 0])

    # 微多普勒时间图（时频图）
    micro_doppler_frames = []
    for frame in rti_framearray:
            # doppler_fft returns (range_bins, doppler_bins), so collect
            # frames by result shape instead of assuming chirps == range bins.
            det_matrix_vis = doppler_fft(frame, window_type_2d=Window.HANNING)
            micro_doppler_frames.append(det_matrix_vis)

    if micro_doppler_frames:
        micro_doppler_data = np.stack(micro_doppler_frames, axis=0)
    else:
        micro_doppler_data = np.zeros((0, numRangeBins, numDopplerBins), dtype=np.float64)
...
def Range_Angle(data,  padding_size=None, clutter_removal_enabled=True, window_type_1d = Window.HANNING,Music_enable = False):
    ...
    # print(frame_SNR)
    # --- capon beamforming
    beamWeights   = np.zeros((VIRT_ANT, BINS_PROCESSED), dtype=np.complex64)
    ...
    # IWR6843ISK 物理天线布局（按你提供的实物图）：
    # RX1 RX2 RX3 RX4 在同一水平线；TX2在上方（俯仰偏移），TX1/TX3在下方（方位主阵列）
    #
    # Radar.cfg chirp发射顺序: chirpCfg txEnable = 1, 4, 2
    # 对应虚拟天线分组（TDMA de-interleave后）:
    #   Vx 0~3   -> 第1组发射(1): TX1
    #   Vx 4~7   -> 第2组发射(4): TX3
    #   Vx 8~11  -> 第3组发射(2): TX2(俯仰TX)
    #
    # 方位角(azimuth): 使用下方两路TX形成的水平阵列，取连续等间距4元子阵 [2,3,4,5]
    # 俯仰角(elevation): 使用同方位位置的上下两层阵元，取 [4,5] (下层) 与 [10,11] (上层)

    # 方位角
    for i in range(BINS_PROCESSED):
        if Music_enable:
            range_azimuth[:, i] = dsp.aoa_music_1D(steering_vec, radar_cube[:, [2, 3, 4, 5], i].T, num_sources=1)
        else:
            range_azimuth[:, i], beamWeights[:, i] = dsp.aoa_capon(
                radar_cube[:, [2, 3, 4, 5], i].T,
                steering_vec,
                magnitude=True,
            )
    ...
    for i in range(BINS_PROCESSED):
        if Music_enable:
            range_elevation[:, i] = dsp.aoa_music_1D(steering_vec, radar_cube[:, [4, 5, 10, 11], i].T, num_sources=1)
        else:
            # [4,5] 与 [10,11] 对应同方位位置的上下层阵元
            # 交替符号用于上下层相位对齐（与当前导向矢量定义保持一致）
            range_elevation[:, i], beamWeights[:, i] = dsp.aoa_capon(
                radar_cube[:, [4, 5, 10, 11], i].T * [[1], [-1], [1], [-1]],
                steering_vec,
                magnitude=True,
            )
...
def extract_pointcloud_from_angle_maps(rai_data, rei_data, range_resolution=None, 
                                       angle_resolution=None, angle_range=None,
                                       threshold_ratio=0.3):
    """
    基于网格检测的点云提取算法
